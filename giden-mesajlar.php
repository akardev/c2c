                            ...
                            <div class="table-responsive ">
                                <table class="table table-dark table-striped">
                                    ...
                                    <tbody>
                                        ...
                                        <?php
                                        $mesajsor = $db->prepare("SELECT mesaj.*, user.* FROM mesaj INNER JOIN user ON mesaj.userGelen=user.userId where mesaj.userGonderen=:id order by mesajZaman DESC");
                                        $mesajsor->execute([
                                            ...
                                            <tr>
                                                <td>
                                                    <a <?php if ($mesajcek['userMagaza'] == 2) { ?> href="satici-<?= seo($mesajcek['userAd'] . "-" . $mesajcek['userSoyad']) . "-" . $mesajcek['userId'] ?>" <?php  } ?>>
                                                        <img width="50" class="img-circle" src="<?php echo $mesajcek['userResim'] ?>" alt=""></a>
                                                </td>
                                                ...
                                                <td><a href="mesaj-detay?gidenmesaj=ok&mesajId=<?php echo $mesajcek['mesajId'] ?>&userGonderen=<?php echo $mesajcek['userGonderen'] ?>"><button class="btn btn-primary btn-xs">Mesajı Gör</button></a></td>
                                                <td><a onclick="return confirm('Bu mesajı silmek istiyor musunuz? \nİşlem geri alınamaz!')" href="sadmin/netting/user-islem.php?gidenmesajsil=ok&mesajId=<?php echo $mesajcek['mesajId'] ?>"><button class="btn btn-danger btn-xs">Sil</button></a>
                                                </td>
                                            </tr>
                                        <?php } ?>
                                    </tbody>
